...
다음 조건을 만족한 뒤 write 기능을 사용하세요.

- 실제 원본이 아닌 TEST 사본 Spreadsheet를 준비했다.
- `VITE_BOOTSTRAP_SPREADSHEET_ID`가 TEST 사본을 가리킨다.
- 대상 Google 계정에 TEST Spreadsheet 편집 권한이 있다.
...
- write는 로그인·접근 권한·시트 구조 검증이 완료된 연결 Sheet에 즉시 반영된다.
