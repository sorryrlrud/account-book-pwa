...
- [ ] 1~7월 예산 화면은 `예산 관리 시작 전`으로 표시되고 해당 월 거래가 8월에 이월되지 않는다.
- [ ] 8월의 전월이월은 0원으로 시작한다.
- [ ] 정산하지 않은 8월 잔여/초과 금액은 9월 실제예산에 자동 반영되지 않는다.
- [ ] 현재 월의 `정산`을 누르면 월 종료 후에만 가능하다는 경고가 뜨고 진행할 수 없다.
- [ ] 종료된 월의 전체 잔액이 남으면 합계를 고정한 채 카테고리별 양수·음수 정산액을 배분할 수 있다.
- [ ] 전체 잔액이 부족하면 카테고리별 부족액을 그대로 이월하거나 0원으로 손실 처리할 수 있다.
- [ ] 정산 완료 후 다음 달 예산 편집 화면이 즉시 열리고 정산액이 반영된다.
- [ ] 새 예산 그룹은 생성한 월부터만 계산되고 이전 월 할당액이 소급 이월되지 않는다.
- [ ] 조회 화면의 카드 확장에는 상세만 보이고 편집 컨트롤은 없다.
- [ ] 최대 예산 입력에 천 단위 콤마가 자동 적용된다.
- [ ] 슬라이더는 5만원, `−/+`는 1만원 단위로 할당액을 변경한다.
- [ ] 할당 합계가 최대 예산을 초과하면 저장 버튼이 비활성화된다.
- [ ] 미할당 예산이 있어도 안내 후 저장할 수 있다.
- [ ] 예산 카테고리 순서 변경·추가·제거가 저장 후 유지된다.
- [ ] 레거시 수동조정이 있던 월은 기존 실제예산을 할당액으로 보존한다.
- [ ] 입력 중 카테고리 선택 시 예산 잔여 1줄 표시가 맞다.

## G. Year links and month zero

- [ ] `2026년 12월 -> 다음`에서 `2027년 1월`이 열린다.
- [ ] 12월 정산 완료 후 연결된 1월 예산 편집이 열리고 12월 정산액이 즉시 반영된다.
